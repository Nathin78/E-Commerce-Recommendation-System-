...
function getTimelineStep(order) {
  ...
  if (hoursSinceOrder < 1) return 0;
  if (hoursSinceOrder < 24) return 1;
  return 2;
}
...
export default function OrderDetailPage() {
  ...
  const activeStep = getTimelineStep(order);
  const steps = ["Confirmed", "Packed", "Shipped"];
      ...
      <Stack direction={{ xs: "column", lg: "row" }} spacing={2} alignItems="flex-start">
        <Box sx={{ flex: 1, width: "100%" }}>
          <Paper sx={{ p: { xs: 2, md: 2.4 }, borderRadius: 3, border: `1px solid ${theme.palette.divider}`, mb: 2 }}>
            <Typography sx={{ fontWeight: 800, mb: 1.8 }}>Order Status</Typography>
            ...
            <Stack spacing={1} sx={{ mt: 2 }}>
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="body2">Your order has been confirmed and recorded successfully.</Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <LocalShippingOutlinedIcon color="info" fontSize="small" />
                <Typography variant="body2">Shipment progress updates will appear here as the order moves forward.</Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
              ...
              {order.items.map((item, index) => (
                <Paper
                  ...
                  sx={{
                    ...
                    bgcolor: theme.palette.mode === "dark" ? "rgba(248,250,252,0.03)" : "#fcfdff"
                  }}
                >
                    ...
                    <Stack direction="row" spacing={1.4} sx={{ minWidth: 0 }}>
                      <Box
                        sx={{
                          ...
                          borderRadius: 2.5,
                          overflow: "hidden",
                          bgcolor: theme.palette.mode === "dark" ? "rgba(96,165,250,0.12)" : "#eef4ff",
                          border: `1px solid ${theme.palette.divider}`,
                          flexShrink: 0
                        }}
                      >
                        <ProductImage
                          src={item.image || productsById[item.productId]?.image}
                          alt={item.name}
        ...
        <Box sx={{ width: { xs: "100%", lg: 350 }, flexShrink: 0 }}>
          <Paper sx={{ p: { xs: 2, md: 2.4 }, borderRadius: 3, border: `1px solid ${theme.palette.divider}`, position: { lg: "sticky" }, top: 92 }}>
            ...
            <Stack spacing={1}>
              ...
              <Typography variant="body2" color="text.secondary">
                Packed: expected within a few hours
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Delivered by: {estimatedDelivery(order)}
              </Typography>
            </Stack>
          </Paper>
        </Box>
